    ...
    while (p < buf + len)
    {
        ...
        case 0x60:
        {
            ...
            else if (type == 0x67)
            {
                ...
                comp->id = p[0];
                comp->data.port = *(uint16_t*)&p[1];
                comp->data.service_data_type = *(uint16_t*)&p[3];
                comp->data.type = p[5];
                comp->data.mime = *(uint32_t*)&p[8];
...
static void process_port(output_t *st, uint16_t port_id, uint8_t *buf, unsigned int len)
{
    ...
    case AAS_TYPE_STREAM:
    {
        ...
        while (len)
        {
                    ...
                    (frame_type != 0xF7 || (port->stream.prev[1] == frame_type && port->stream.prev[2] == 0xFF)))
            {
                ...
                {
                    port->stream.idx--;
                    log_debug("Stream data: port=%04X type=%d size=%d size2=%d", port_id, port->stream.type, port->stream.idx, swap16(*(uint16_t *)port->stream.data));
                }
                port->stream.idx = 0;
    ...
    case AAS_TYPE_LOT:
    {
        ...
        uint16_t lot = *(uint16_t*)&buf[2];
        uint32_t seq = *(uint32_t*)&buf[4];
        ...
        if (seq == 0)
        {
            ...
            // uint32_t size
            // uint32_t mimeHash
            file->size = *(uint32_t*)&buf[8];
            file->mime = *(uint32_t*)&buf[12];
            buf += 16;
            len -= 16;
            hdrlen -= 16;
